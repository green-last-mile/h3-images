...
@click.option('--tile_res', default=512, type=int, help='Resolution of the tile (square).')
@click.option('--output_dir', default='output', type=str, help='Directory to save the generated images.')
def main(csv_file, style, tile_res, output_dir):
    ...
    for _, row in list_df.iterrows():
        ...
        # Prepare the command
        command = [
            'python', str(script_path), '--h3', h3_index, '--output', str(output_file),
            '--tile-res', str(tile_res), '--style', style
        ]
